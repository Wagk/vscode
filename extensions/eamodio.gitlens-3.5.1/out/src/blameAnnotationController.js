...
class BlameAnnotationController extends vscode_1.Disposable {
    ...
    _onConfigurationChanged() {
        let toggleWhitespace = vscode_1.workspace.getConfiguration(`${constants_1.ExtensionKey}.advanced.toggleWhitespace`).get('enabled');
        if (!toggleWhitespace) {
        ...
        const cfg = vscode_1.workspace.getConfiguration(constants_1.ExtensionKey).get('blame');
        ...
        for (const [key, p] of this._annotationProviders) {
            ...
            setTimeout(() => {
                if (e.document.isDirty)
                    return;
                logger_1.Logger.log('TextDocumentChanged:', `Clear blame annotations for column ${key}`);
                this.clear(key);
            }, 1);
        }
    }
